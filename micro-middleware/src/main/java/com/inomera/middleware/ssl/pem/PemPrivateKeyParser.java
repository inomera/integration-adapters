...
import com.inomera.middleware.ssl.pem.PemPrivateKeyParser.DerElement.TagType;
import com.inomera.middleware.ssl.pem.PemPrivateKeyParser.DerElement.ValueType;
...
public class PemPrivateKeyParser {
  ...
  static {
    List<PemPrivateKeyParser.PemParser> parsers = new ArrayList<>();
    parsers.add(new PemPrivateKeyParser.PemParser(PKCS1_RSA_HEADER, PKCS1_RSA_FOOTER, PemPrivateKeyParser::createKeySpecForPkcs1Rsa,
        "RSA"));
    parsers.add(new PemPrivateKeyParser.PemParser(SEC1_EC_HEADER, SEC1_EC_FOOTER, PemPrivateKeyParser::createKeySpecForSec1Ec, "EC"));
    parsers.add(new PemPrivateKeyParser.PemParser(PKCS8_HEADER, PKCS8_FOOTER, PemPrivateKeyParser::createKeySpecForPkcs8, "RSA",
        "RSASSA-PSS", "EC", "DSA", "EdDSA", "XDH"));
    parsers.add(new PemPrivateKeyParser.PemParser(PKCS8_ENCRYPTED_HEADER, PKCS8_ENCRYPTED_FOOTER,
  ...
  private static PKCS8EncodedKeySpec createKeySpecForSec1Ec(byte[] bytes, String password) {
    PemPrivateKeyParser.DerElement ecPrivateKey = PemPrivateKeyParser.DerElement.of(bytes);
    Assert.state(ecPrivateKey.isType(ValueType.ENCODED, TagType.SEQUENCE),
        "Key spec should be an ASN.1 encoded sequence");
    PemPrivateKeyParser.DerElement version = PemPrivateKeyParser.DerElement.of(ecPrivateKey.getContents());
    ...
    PemPrivateKeyParser.DerElement privateKey = PemPrivateKeyParser.DerElement.of(ecPrivateKey.getContents());
    ...
    PemPrivateKeyParser.DerElement parameters = PemPrivateKeyParser.DerElement.of(ecPrivateKey.getContents());
    return createKeySpecForAlgorithm(bytes, EC_ALGORITHM, getEcParameters(parameters));
  }

  private static int[] getEcParameters(
      PemPrivateKeyParser.DerElement parameters) {
    if (parameters == null) {
      return EC_PARAMETERS;
    }
    Assert.state(parameters.isType(ValueType.ENCODED), "Key spec should contain encoded parameters");
    PemPrivateKeyParser.DerElement contents = PemPrivateKeyParser.DerElement.of(parameters.getContents());
  ...
  private static PKCS8EncodedKeySpec createKeySpecForAlgorithm(byte[] bytes, int[] algorithm, int[] parameters) {
    try {
      PemPrivateKeyParser.DerEncoder encoder = new PemPrivateKeyParser.DerEncoder();
      encoder.integer(0x00); // Version 0
      PemPrivateKeyParser.DerEncoder algorithmIdentifier = new PemPrivateKeyParser.DerEncoder();
  ...
  private static PKCS8EncodedKeySpec createKeySpecForPkcs8Encrypted(byte[] bytes, String password) {
    return PemPrivateKeyParser.Pkcs8PrivateKeyDecryptor.decrypt(bytes, password);
  }
  ...
  static PrivateKey parse(String text, String password) {
    ...
    try {
      for (PemPrivateKeyParser.PemParser pemParser : PEM_PARSERS) {
  ...
  static class DerEncoder {
    ...
    byte[] toSequence() throws IOException {
      PemPrivateKeyParser.DerEncoder sequenceEncoder = new PemPrivateKeyParser.DerEncoder();
  ...
  static final class DerElement {
    ...
    private final PemPrivateKeyParser.DerElement.ValueType valueType;
    ...
    private DerElement(ByteBuffer bytes) {
      byte b = bytes.get();
      this.valueType = ((b & 0x20) == 0) ? PemPrivateKeyParser.DerElement.ValueType.PRIMITIVE : PemPrivateKeyParser.DerElement.ValueType.ENCODED;
    ...
    boolean isType(
        PemPrivateKeyParser.DerElement.ValueType valueType) {
      return this.valueType == valueType;
    }

    boolean isType(
        PemPrivateKeyParser.DerElement.ValueType valueType, PemPrivateKeyParser.DerElement.TagType tagType) {
    ...
    static PemPrivateKeyParser.DerElement of(byte[] bytes) {
      return of(ByteBuffer.wrap(bytes));
    }

    static PemPrivateKeyParser.DerElement of(ByteBuffer bytes) {
      return (bytes.remaining() > 0) ? new PemPrivateKeyParser.DerElement(bytes) : null;
    }
